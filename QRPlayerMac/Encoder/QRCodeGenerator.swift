//From https://github.com/sbhklr/QRCoder/blob/master/Pod/Classes/QRCodeGenerator.swift
...
public class QRCodeGenerator : NSObject {
    ...
    public var correctionLevel:CorrectionLevel = .L
    ...
    public func createImage(value:String, size:CGSize) -> QRImage? {
        let stringData = value.data(using: String.Encoding.utf8, allowLossyConversion: true)
        if let qrFilter = CIFilter(name: "CIQRCodeGenerator") {
            ...
            let transform = CGAffineTransform.init(scaleX: size.width/outputSize.width, y: size.height/outputSize.height)
            let outputScaled = outputImage.transformed(by: transform)
            let rep: NSCIImageRep = NSCIImageRep(ciImage: outputScaled)
...
//            return createNonInterpolatedImageFromCIImage(image: outputImage, size: size)
    ...
    private func createNonInterpolatedImageFromCIImage(image:CIImage, size:CGSize) -> QRImage? {
        ...
        graphicsContext.draw(cgImage, in:  CGRect(x: 0.0,y: 0.0,width:size.width,height:size.height))
        return newImage
    }
    #endif
